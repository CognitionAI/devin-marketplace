...
2. If the target is not registered, offer to call `create_domain` or `create_repositories`. A new domain must pass `verify_domain` before a scan can run against it.
...
- For recurring scans, call `create_schedule` with a cron expression and a timezone. Set `also_run_now` only when the user wants a scan immediately.
